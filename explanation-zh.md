...
| 代码 | 英文                 | 中文含义     | 说明                                           |
| :--- | :------------------- | :----------- | :--------------------------------------------- |
| `DP` | Danger Potential     | 潜在危险     | 可能导致危险情况的错误，通常扣分较重。         |
| `LS` | Lack of Skill        | 缺乏技巧     | 显示驾驶技巧不足但非即时危险的错误，扣分较轻。 |
| `CP` | Congestion Potential | 可能造成拥堵 | 不必要地阻碍交通流畅的错误，扣分较轻。         |

## 三、 考试项目及扣分点详解

* **注意:** 下表中对具体错误代码的中文解释是基于常见的驾驶错误和随附文件上下文进行的推断。最准确的定义请参考华盛顿州交通部 (WA DOL) 的官方材料。

| 考试项目 (英文)                  | 考试项目 (中文) | 考察技能说明                                     | 扣分类型 | 错误代码 (缩写)及推测中文解释                                                                                                                                                                                          | 分类最高扣分 |
| :------------------------------- | :-------------- | :----------------------------------------------- | :------- | :--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :----------- |
...
| **Traffic control devices** | **交通控制标识** | 识别并正确响应交通标志（如限速、警告等）。         | **DP** | `Ftc`(未能遵守标识要求, 如超速)                                                                                                                                                                                                                                                                                                     | 4            |
...
| **Mechanical operation** | **机械操作** | 正确使用车辆控制部件（方向盘、踏板、档位等）。       | **DP** | `Sig`(信号使用错误/缺失), `Hands`(方向盘手势不当), `Arm`(手臂信号不当), `Pos`(坐姿影响控制), `1Hand`(过度单手驾驶), `Clutch`(离合操作差), `Gear`(档位使用错误), `Brk`(刹车使用不当-过猛/软)                                                                                                                                               | 2            |
...
| **Uncontrolled intersections** | **无控制路口** | 在没有交通信号/标志的路口，正确接近并通行。          | **DP** | `Vis`(观察/扫视不足), `Spd`(速度不当/过快)                                                                                                                                                                                                                                                                                          | 4            |
...
*免责声明：本文档内容基于用户提供的计分表文件及通用驾驶知识进行解释和整理。所有官方定义和规则请以华盛顿州交通部 (Washington State Department of Licensing, WA DOL) 发布的最新版驾驶员指南和相关规定为准。*
